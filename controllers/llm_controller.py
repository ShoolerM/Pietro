...
class LLMController:
    ...
    def update_model(self, model_name):
        ...
        """
        if model_name and not model_name.startswith("Error:") and model_name != "No models available":
            try:
                self.llm = ChatOpenAI(
                    base_url=self.llm_model.base_url,
                    model=model_name,
    ...
    def invoke_llm(self, query, system_prompt, append_text_callback, 
                   append_thinking_callback, render_markdown_callback,
                   set_waiting_callback, set_stop_enabled_callback):
    ...
    def _invoke_thread(self, query, system_prompt, signals):
        ...
        try:
            ...
            # Use signal to trigger markdown rendering on UI thread
            ...
            try:
                # Use signal to disable stop button on UI thread
    ...
    def compress_rolling_summary(self, current_rolling_summary: str, new_chunk_summary: str, 
                                  max_tokens: int, append_thinking_callback) -> tuple[str, int]:
        ...
        try:
            ...
            append_thinking_callback(f"\n❌ Rolling summary compression failed: {e}\n")
    ...
    def process_story_with_summarization(self, current_story, max_raw_tokens, 
                                         max_rolling_summary_tokens, summary_model,
                                         thinking_callback, completed_callback, error_callback,
        ...
        thread = threading.Thread(
            ...
            args=(current_story, max_raw_tokens, max_rolling_summary_tokens, 
                  summary_model, signals),
            daemon=True
        )
        thread.start()
    
    def _process_story_thread(self, current_story, max_raw_tokens, 
                              max_rolling_summary_tokens, summary_model, signals):
        ...
        try:
            ...
            if split_pos > 0:
                ...
                signals.thinking_signal.emit(
                    f"📦 Processing {older_tokens} tokens of older content...\n\n"
                )
                ...
                for idx, (chunk_text, start, end) in enumerate(chunks):
                    ...
                    signals.thinking_signal.emit(
                        f"\n🔄 Chunk {idx+1}/{len(chunks)} ({chunk_tokens} tokens)...\n"
                    )
                    ...
                    if combined_tokens > max_rolling_summary_tokens and rolling_summary:
                        signals.thinking_signal.emit(
                            f"  ⚙️ Compressing rolling summary ({combined_tokens} → {max_rolling_summary_tokens})...\n"
                        )
    ...
    def generate_story_chunk(self, query, system_prompt, paragraph_limit,
                             append_text_callback, append_thinking_callback,
                             render_markdown_callback, set_waiting_callback,
    ...
    def _chunk_generation_thread(self, query, system_prompt, paragraph_limit, signals):
        ...
        try:
            ...
            # Normal termination when paragraph limit reached or user stopped
            pass
        except Exception as e:
            # Use signal to append error text on UI thread
            signals.text_signal.emit(f"\n[error] {e}\n")
