...
class PromptsPanel(QtWidgets.QWidget):
    ...
    def _create_supplemental_tab(self):
        ...
        self.supp_list.itemDoubleClicked.connect(
            self._on_supplemental_file_double_clicked
        )
    ...
    def _create_rag_tab(self):
        ...
        self.rag_create_db.clicked.connect(
            lambda: self.rag_create_database_clicked.emit()
        )
    ...
    def _on_delete_database_clicked(self):
        ...
        if not checked_databases:
            ...
            self.show_warning(
                "No Selection", "Please check at least one database to delete."
            )
            return
        ...
        print(
            f"Asking user to confirm deletion of {len(checked_databases)} database(s)"
        )
        ...
        for file_path, is_dir in files:
            ...
            if is_dir:
                folder_item = QtWidgets.QTreeWidgetItem(
                    parent_item if parent_item else tree_widget
                )
    ...
    def _show_supp_context_menu(self, position):
        ...
        if item:
            ...
            create_file_action = menu.addAction("Create File")
            create_file_action.triggered.connect(
                lambda: self._on_create_supp_file(None)
            )
            create_folder_action = menu.addAction("Create Folder")
            create_folder_action.triggered.connect(
                lambda: self._on_create_supp_directory(None)
            )
        ...
        refresh_action.triggered.connect(
            lambda: self.supplemental_refresh_clicked.emit()
        )
    ...
    def _show_sys_context_menu(self, position):
        ...
        if item:
            ...
            if item_data:  # It's a file
                delete_action = menu.addAction("Delete File")
                delete_action.triggered.connect(
                    lambda: self._on_system_delete_clicked(item)
                )
            else:  # It's a folder
                create_file_action = menu.addAction("Create File")
                create_file_action.triggered.connect(
                    lambda: self._on_create_sys_file(item)
                )
                create_folder_action = menu.addAction("Create Folder")
                create_folder_action.triggered.connect(
                    lambda: self._on_create_sys_directory(item)
                )
                menu.addSeparator()
                delete_action = menu.addAction("Delete Folder")
                delete_action.triggered.connect(
            ...
            create_folder_action = menu.addAction("Create Folder")
            create_folder_action.triggered.connect(
                lambda: self._on_create_sys_directory(None)
            )
