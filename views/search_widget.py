...
from models.stylesheets import SEARCH_BAR
...
class SearchWidget(QtWidgets.QWidget):
    ...
    def _highlight_current_match(self):
        ...
        current_format.setBackground(
            QtGui.QColor(200, 200, 0, 150)
        )  # Bright yellow, fully opaque
    ...
    def _replace_all(self):
        ...
        try:
            if use_regex:
                flags = 0 if case_sensitive else re.IGNORECASE
                new_content = re.sub(
                    search_text, replacement_text, content, flags=flags
                )
            else:
                if not case_sensitive:
